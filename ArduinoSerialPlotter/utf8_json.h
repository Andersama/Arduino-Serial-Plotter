...
namespace utf8_json {
inline unsigned int mask8(char const value) { return value & 0xff; }

inline bool is_valid_continuation_byte(unsigned int byte) { return ((byte & 0xC0) == 0x80); }
...
std::vector<unsigned int> decode_utf8(const std::string &input, const int replacement = 0xfffd) {
    ...
    for (std::string::const_iterator iterator = input.begin(); iterator != input.end(); ++iterator) {
        ...
        if (code_unit1 < 0x80) {
            ...
            if (!is_valid_continuation_byte(code_unit2) || (code_unit1 == 0xE0 && code_unit2 < 0xA0)) /* overlong */ {
                insert_replacement(codepoints, replacement, 2);
            } else {
            ...
            code_unit2 = get_next_byte(iterator, input.end());
            if (!is_valid_continuation_byte(code_unit2) || (code_unit1 == 0xF0 && code_unit2 < 0x90) || /* overlong */
                ...
                if (!is_valid_continuation_byte(code_unit3)) {
                    ...
                    if (!is_valid_continuation_byte(code_unit4)) {
                        ...
                        codepoints.push_back((code_unit1 << 18) + (code_unit2 << 12) + (code_unit3 << 6) + code_unit4 -
                                             0x3C82080);
                    }
                }
            }
...
std::string json_encode_codepoints(std::vector<unsigned int> const &codepoints) {
    ...
    for (std::vector<unsigned int>::const_iterator codepoint = codepoints.begin(); codepoint != codepoints.end();
         ++codepoint) {
        if (*codepoint == 8) { // \b
